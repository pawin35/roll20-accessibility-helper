...
(function () {
  ...
  /**
   * The default, and it is **polite**: it queues rather than interrupting, so
   * an incidental line does not cut across whatever the user is reading.
   *
   * Making *everything* assertive was tried in 1.39.0 and reverted — it turned
   * every other player's chat into an interruption, and it did not fix the
  ...
  /**
   * The assertive twin. Reserved for things the user asked for and is waiting
   * on — a roll they just fired — because assertive *interrupts* whatever is
   * being spoken. That is exactly the point: the result must not queue behind
   * the context a screen reader reads out when focus lands back on a control
   * inside a table. Everything else stays polite.
   *
   * Only two callers, both on the claimed-result path: `deliverClaimed()` and
   * the sheet frame's `r20a11ySpeak` receiver. Widening that list is how 1.39.0
   * went wrong — keep it to what the user is actively waiting for.
   */
  function getAlertRegion() {
    if (alertRegion && alertRegion.isConnected) return alertRegion;
    alertRegion = document.createElement("div");
    alertRegion.setAttribute("aria-live", "assertive");
    alertRegion.setAttribute("role", "alert");
    alertRegion.className = CLASS_PREFIX + "-live-region";
  ...
  function announce(message, assertive) {
    const region = assertive ? getAlertRegion() : getLiveRegion();
  ...
  // --- Speaking in the frame the user is in ------------------------------
  //
  // The VTT's chat log lives in the top frame, so that is where a roll result
  // is announced — but when a shortcut was pressed inside the floating
  // character sheet, that is not where the user is. A live region in another
  // document is not reliably read — and it is the wrong frame to interrupt from
  // when focus has just landed back on a control there.
  //
  // So the announcement is *routed*, not echoed: while a route is set the top
  // frame does not announce locally at all, it posts the line down and the
  // sheet frame speaks it. One line then clears the route, so an unrelated
  // message a moment later is announced normally again.

  const SHEET_FRAME_ORIGIN = "https://advanced-sheets.production.roll20preflight.net";
  const TOP_FRAME_ORIGIN = "https://app.roll20.net";
  const CLAIM_CEILING_MS = 5000;
  ...
  /**
   * Claim the next chat announcement as the answer to something the user just
   * pressed: deliver it to `frame` if the key came from there, and speak it
   * assertively either way so it cuts through the context a screen reader reads
   * out when focus lands back on their control.
   *
   * Cleared once one line has gone, or after a ceiling, so an unrelated message
   * a moment later is announced normally again.
   */
  function claimNextAnnouncement(frame) {
    clearClaim();
    claimed = true;
    claimFrame = frame || null;
    claimTimer = window.setTimeout(clearClaim, CLAIM_CEILING_MS);
  }

  /**
   * True when `text` has been dealt with and the caller must not announce it
   * as well — routed *or* echoed would say it twice.
   */
  function deliverClaimed(text) {
    if (!claimed || !text) return false;
    const frame = claimFrame;
    clearClaim();
    if (frame) {
      try {
        frame.postMessage({ r20a11ySpeak: text }, SHEET_FRAME_ORIGIN);
        return true;
      } catch (e) {
        /* frame unreachable; speak it here instead */
      }
    }
    announce(text, true);
    return true;
  ...
  // The receiving half. Only ever in the sheet iframe — the character-sheet
  // page's own top frame is `window.top` and never gets these.
  if (window.top !== window) {
    window.addEventListener("message", (event) => {
      if (event.origin !== TOP_FRAME_ORIGIN) return;
      const text = event.data && event.data.r20a11ySpeak;
      // Assertive: this is the answer to something the user just pressed, and
      // it must cut through the context spoken when focus returned to their
      // control rather than wait behind it.
      if (typeof text === "string" && text) announce(text, true);
    });
  }
   ...
   * Roll20 chose.
   *
   * Shared rather than duplicated per feature: every shortcut that rolls has to
   * agree on who it is rolling as, and two copies of this could drift apart.
   * Returns "" anywhere the dropdown does not exist, which is every frame but
   * the VTT's top one.
   */
  function currentCharacterName() {
    const select = document.querySelector("#speakingas");
    if (!select || !select.options) return "";
    const names = [];
    for (const option of select.options) {
      if (String(option.value || "").indexOf("character|") === 0) {
  ...
  // --- Roll20's number steppers ----------------------------------------
  //
  // The sheet ships *two* structurally identical stepper components and uses
  // both: the original `PolyIncrementer` and a newer `UtilityIncrementer`.
  // Ability scores moved to the new one and every feature that named the old
  // one stopped matching — silently, because only abilities-table.js reports a
  // failure. Both are still in the bundle, so this is an addition and not a
  // rename: every selector accepts either, and there is one place to add the
  // third when Roll20 ships it.
  //
  // The two differ in one way that matters: PolyIncrementer's caption is a
  // `<label role="label">`, and `role="label"` is not a real ARIA role, so it
  // overrides the element's own semantics and names nothing. UtilityIncrementer
  // drops the role, so its `for`/`id` pair works and its input is already
  // named. Naming it again is harmless and more explicit.
  const INCREMENTER = {
    BOX: '[data-testid="poly-incrementer"], [data-testid="utility-incrementer"]',
    LABEL: ".poly-incrementer__label, .utility-incrementer__label",
    INPUT: ".poly-incrementer__input, .utility-incrementer__input",
    INCREASE:
      ".poly-incrementer__button--increment, .utility-incrementer__button--increment",
    DECREASE:
      ".poly-incrementer__button--decrement, .utility-incrementer__button--decrement",
  };
  ...
  // --- "Table ready" signal ----------------------------------------------
  //
  // Features call `markReady("chat")` / `markReady("speaking")` when they are
  // up; once all have reported, one chime and one spoken "Table ready."
  // announce that the page is safe to drive. This is what stops a user pressing
  // a shortcut before the VTT has finished loading. It is inert anywhere else:
  // only the VTT top frame ever sets these flags.
  //
  // NOTE: the battle-grid feature (`features/map-grid.js`) is currently
  // soft-disabled in the manifest, so `markReady("grid")` never fires and
  // `grid` is not in the gate. When map-grid is re-enabled, add `grid: false`
  // back to `readyFlags` and `!readyFlags.grid` back to `checkReady()`.

  const readyFlags = { chat: false, speaking: false };
  let readyDone = false;
  ...
  function checkReady() {
    if (readyDone || !readyFlags.chat || !readyFlags.speaking) return;
    readyDone = true;
    readyChime();
