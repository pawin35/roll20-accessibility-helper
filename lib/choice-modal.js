...
(function () {
  ...
  let options = [];
  let activeIndex = 0;
  // "list" or "slider" — which control is in use, and so what commit() reads.
  let mode = "list";
  let lastFocus = null;
  let onCommit = null;
  let onClose = null;
  ...
  function build() {
    ...
    // A native <input type="range">, not a custom role="slider". Arrow keys,
    // Home/End and Page Up/Down all work without any code, and a screen reader
    // announces the value on every change — which for hit points is the whole
    // point of the control.
    slider = document.createElement("input");
    slider.type = "range";
    slider.step = "1";
    slider.className = CLASS_PREFIX + "-slider";
    slider.setAttribute("aria-labelledby", labelEl.id);
    ...
    slider.addEventListener("input", describeSlider);
  ...
  /**
   * "9 of 20" rather than a bare "9". `aria-valuetext` replaces the number a
   * screen reader would otherwise read, and hit points mean little without the
   * maximum beside them.
   */
  function describeSlider() {
    slider.setAttribute(
      "aria-valuetext",
      slider.value + " of " + slider.max
    );
  }
  ...
  function commit() {
    if (mode === "slider") return close(true, slider.value);
   ...
   */
  /**
   * Everything both modes do: the shell, the wiring, and the record of where
   * focus came from. Returns false when a dialog is already up.
   */
  function begin(config, which) {
    build();
    ...
    // Only the control in use is present for a screen reader; the other would
    // otherwise be an extra tab stop announcing a stale value.
    listbox.hidden = which !== "list";
    slider.hidden = which !== "slider";
  ...
  /**
   * A slider over a numeric range — used for hit points, where the choice is a
   * number on a scale rather than one of a list.
   */
  function openSlider(config) {
    if (!begin(config, "slider")) return false;

    const min = Number(config.min) || 0;
    const max = Number(config.max);
    // A range whose max is not above its min gives a control that cannot move.
    if (!(max > min)) return false;

    slider.min = String(min);
    slider.max = String(max);
    slider.value = String(Math.min(max, Math.max(min, Number(config.value) || 0)));
    describeSlider();
  ...
  function open(config) {
    ...
    if (!begin(config, "list")) return false;
  ...
  window.Roll20A11y.choiceModal = { open, openSlider, close, isOpen };
})();
