...
(function () {
  ...
  function openHere(config, afterCommit) {
    ...
    const show = config.kind === "slider" ? choiceModal.openSlider : choiceModal.open;
    return show({
      title: config.title,
      label: config.label,
      items: config.items,
   ...
   */
  /**
   * A numeric slider instead of a list. Same plumbing throughout — it travels
   * as a `kind` on the same message and comes back through the same reply — so
   * the focus dance, the silencer lead and the announcement claim are all the
   * one implementation.
   */
  function openSlider(config) {
    return open(Object.assign({}, config, { kind: "slider" }));
  }
  ...
  window.Roll20A11y.remoteModal = { open, openSlider, fail };
})();
